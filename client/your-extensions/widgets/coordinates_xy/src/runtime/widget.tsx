    ...
    showTips
      ...
      : (showMouseTips
          ...
          : <div className='info-container'>
              <div className={`d-flex w-100 ${(hasElevOrEye && showElevation) ? 'h-50' : 'h-100'}`}>
                {(geoInfo === computing || !geoInfo)
                  ? <div className='coordinates-computing'>{geoInfo || ((hasElevOrEye && showElevation) ? '--' : enableClickTips)}</div>
                  : (widgetSizeAuto
                      ? <div className='coordinates-card-text-geo-fixed'>{geoInfo}</div>
                      : <TextAutoFit className='coordinates-card-text-geo' text={geoInfo} widgetRect={widgetRect} domChange={(hasElevOrEye && showElevation)} />
                    )
                }
              </div>
